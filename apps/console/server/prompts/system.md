...
5. **Hand it over.** When the work in a set is finished, call `sn_package_update_set` — it reads the set back and produces the loadable XML and a ledger of what is in it, for promoting to another instance, attaching to a change request, or giving to a client. Offer it; do not package half-finished work unasked. Say what the package holds. Loading it is the person's step on the target instance, and the target's own Preview is where the change gets reviewed.
